                ...
                @foreach($conversations as $conversation)
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <a href="#conversation-{{ $conversation->id }}" class="stretched-link"
                               data-toggle="collapse"
                               role="button" aria-expanded="false"
                               aria-controls="conversation-{{ $conversation->id }}"></a>
                        </div>
                    </div>
                @endforeach
            @else
                            ...
                            @if(count($conversation->messages) > 0)
                                @foreach($conversation->messages as $message)
                                    <div class="card mb-3 message" data-id="{{ $message->id }}"
                                         data-user="{{ $message->user_id == auth()->id() ? 'me' : 'other' }}"
                                        ...
                                        <div class="card-body"
                                            ...
                                            <small
                                                style="opacity: .5; font-size: .7rem">{{$message->created_at}}</small>
                                        </div>
                                    </div>
                                @endforeach
                            @else
